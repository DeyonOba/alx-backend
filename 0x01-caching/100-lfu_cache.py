...
caching system:
    ...
    - def put(self, key, item):
        ...
        `BaseCaching.MAX_ITEMS`:
            - you must discard the least frequently used
            item using (LFU algorithm).
...
class LFUCache(BaseCaching):
    """
    LFUCache inherits from BaseCaching and is a caching system that
    implements [Least frequency used] caching algorithm.
    """
    def __init__(self):
        super().__init__()
        self.cache_history = []
    ...
    def cache_miss(self, key):
        ...
        if len(self.cache_history) == self.MAX_ITEMS:
            discarded_key = self.find_least_frequently_used()
            self.cache_data.pop(discarded_key)
            self.cache_frequency.pop(discarded_key)
            self.cache_history.remove(discarded_key)
            print("DISCARD: {}".format(discarded_key))
        self.cache_history.append(key)
        self.cache_frequency.update({key: 0})
